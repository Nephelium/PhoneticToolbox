...
def parse_textgrid(path: Path) -> Optional[TextGrid]:
    ...
    if not lines or 'File type = "ooTextFile"' not in lines[0]:
        # Sometimes line[0] might have BOM or slight diffs, check containment
        if not any('File type = "ooTextFile"' in l for l in lines[:2]):
            return None
