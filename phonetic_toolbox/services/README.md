...
3.  **核心参数提取**:
    ...
    *   **F0**: 调用 `core.acoustic.compute_praat_f0` 和 `compute_reaper_f0`。
4.  **频谱参数计算 (Batch)**:
...
7.  **输出**: 返回包含所有对齐数据的 `pandas.DataFrame`。
...
# 3. 执行分析
# 结果 df 包含了 F0, Formants, H1-A3, Jitter, Shimmer 等所有列
try:
    df = service.analyze_file(
        wav_path="test.wav",
        config=config,
    ...
    # 4. 保存结果
    from phonetic_toolbox.services.io.excel import save_excel
    save_excel("result.xlsx", df.to_dict(orient='list'))
