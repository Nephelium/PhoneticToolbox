...
class EpochTrackerPy:
    ...
    def resample_and_return_results(self, resample_interval):
        if (self.sample_rate <= 0.0):
            print(f"REAPER DEBUG: sample_rate {self.sample_rate} <= 0")
            return None, None
        if (len(self.output) == 0):
            print("REAPER DEBUG: len(self.output) == 0")
            return None, None
        if resample_interval <= 0.0:
            print(f"REAPER DEBUG: resample_interval {resample_interval} <= 0")
            return None, None
        ...
        if n_frames <= 0:
            print(f"REAPER DEBUG: n_frames {n_frames} <= 0. last_time={last_time}, interval={resample_interval}")
            return None, None
        ...
        # self.output is in reverse time order (latest first)
        # We iterate from limit (earliest) to 0 (latest)
        prev_frame = -1
        prev_f0 = 0.0
        prev_corr = 0.0
        
        for i in range(limit, -1, -1):
            t = self.output[i]['resid_index'] / self.sample_rate
            frame = self._round_up(t / resample_interval)
            ...
            # Fill gaps logic:
            # If we skipped frames between prev_frame and current frame, fill them.
            # We assume continuous voicing if gap is small? 
            # REAPER usually fills unvoiced regions with 0.
            # But if we have voiced segments separated by frames, we should check 'voiced' flag?
            # self.output[i] has 'voiced' flag.
            
            # Actually, REAPER resample usually just puts the nearest epoch's F0 into the frame?
            # Or does it interpolate?
            # The original C++ code (and this Python port) seems to try to fill.
            
            if (prev_frame != -1) and (frame > prev_frame + 1):
                # If both current and previous were voiced, maybe interpolate?
                # For now, let's just leave zeros (unvoiced) in gaps, 
                # unless we want to hold the previous value.
                # The original Python code had some logic for this.
                pass
