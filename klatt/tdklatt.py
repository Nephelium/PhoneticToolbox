...
try:
    ...
    print("Please make sure that math, numpy, scipy, and sounddevice are installed.")
    import sys
    sys.exit()
...
class KlattParam1980(object):
    ...
    def __init__(self, FS=10000, N_FORM=5, DUR=1, F0=100,
                       ...
                       Jitter=0, Shimmer=0, SHR=0, HNR=None, Slope=0):
        self.FS = FS
        self.DUR = DUR
        self.N_FORM = N_FORM
        ...
        self.Jitter = np.ones(self.N_SAMP)*Jitter
        self.Shimmer = np.ones(self.N_SAMP)*Shimmer
        self.SHR = np.ones(self.N_SAMP)*SHR
        self.Slope = np.ones(self.N_SAMP)*Slope
        if HNR is not None:
            self.HNR = np.ones(self.N_SAMP)*HNR
        else:
            self.HNR = None
...
class KlattSynth(object):
    ...
    def __init__(self):
        ...
        param_list = ["F0", "AV", "OQ", "SQ", "TL", "FL", # Source
                      ...
                      "Jitter", "Shimmer", "SHR", "HNR", "Slope"] # New Params
    ...
    def play(self):
        ...
        """
        sd.play(self.output, self.params["FS"])
...
class KlattVoice1980(KlattSection):
    ...
    def __init__(self, mast):
        ...
        self.components = [self.impulse, self.rgp, self.rgz, self.rgs, \
                           self.av, self.avs, self.mixer, self.tilt, self.switch]
    ...
    def patch(self):
        ...
        self.mixer.connect([self.tilt])
        self.tilt.connect([self.switch])
        self.switch.connect([*self.outs])

    def do(self):
        self.impulse.impulse_gen(F0=self.mast.params["F0"],
                                 Jitter=self.mast.params["Jitter"],
                                 SHR=self.mast.params["SHR"])
        ...
        # Apply Shimmer to AV
        av_db = self.mast.params["AV"].copy()
        if self.mast.params["Shimmer"] is not None:
            shimmer_val = self.mast.params["Shimmer"]
            # Check if scalar or array
            if np.isscalar(shimmer_val) and shimmer_val == 0:
                pass
            else:
                noise = (np.random.random(len(av_db)) - 0.5) * 2 * shimmer_val
                av_db += noise
                
        self.av.amplify(dB=av_db)
        self.avs.amplify(dB=self.mast.params["AVS"])
        self.mixer.mix()
        self.tilt.filter(slope=self.mast.params["Slope"])
...
class Impulse(KlattComponent):
    ...
    def impulse_gen(self, F0, Jitter=None, SHR=None):
        """
        Generates impulse train with Jitter and Diplophonia (SHR).

        Arguments:
            F0 (arrry): Array of F0 values at each sample
            Jitter (array): Array of Jitter values (percent)
            SHR (array): Array of SHR values (0-1 amplitude reduction)
        """
        fs = self.mast.params["FS"]
        ...
        f0_init = F0[0] if len(F0) > 0 else 100
        if f0_init <= 0: f0_init = 100
        
        while n < self.mast.params["N_SAMP"]:
            # Place pulse at n
            amp = 1.0
            
            # Apply SHR (Diplophonia) - Attenuate every second pulse
            if SHR is not None:
                shr_val = SHR[n] if n < len(SHR) else SHR[-1]
                if pulse_count % 2 == 1:
                    # SHR value (e.g., 0.5) reduces amplitude
                    # If SHR=0, amp=1. If SHR=1, amp=0?
                    amp = max(0.0, 1.0 - shr_val)
            
            if n < self.mast.params["N_SAMP"]:
                self.output[n] = amp
            
            # Determine next period length
            # Get F0 at current time
            f0_val = F0[n] if n < len(F0) else F0[-1]
            if f0_val <= 1: f0_val = 1
            
            period_samples = fs / f0_val
            
            # Apply Jitter
            if Jitter is not None:
                jit_val = Jitter[n] if n < len(Jitter) else Jitter[-1]
                if jit_val > 0:
                    perturb = (np.random.random() - 0.5) * 2 * (jit_val / 100.0)
                    period_samples = period_samples * (1.0 + perturb)
            
            period_int = int(round(period_samples))
            if period_int < 1: period_int = 1
...
class SpectralTilt(KlattComponent):
    """
    One-pole lowpass filter for spectral tilt.
    y[n] = (1-a)*x[n] + a*y[n-1]
    """
    def __init__(self, mast):
        KlattComponent.__init__(self, mast)
        
    def filter(self, slope):
        """
        slope (array): Slope in dB (attenuation at 3000 Hz).
        If slope is 0, a=0 (no filter).
        """
        # Convert dB slope to alpha
        # TLT (dB) = slope
        # |H(3000)|^2 = 10^(-TLT/10)
        # alpha is derived from TLT
        
        fs = self.mast.params["FS"]
        f_cut = 3000.0
        omega = 2 * np.pi * f_cut / fs
        cos_w = np.cos(omega)
        k = 1.0 - cos_w
        
        # G = 10^(-slope/20)
        # To avoid division by zero if slope is 0 (G=1), handle separately or carefully
        
        # Helper to calculate alpha from slope
        def slope_to_alpha(s):
            if s <= 1e-6:
                return 0.0
            
            G = 10.0 ** (-s / 20.0)
            
            # Equation: (1-G^2)(1-alpha)^2 = 2*k*G^2*alpha
            # Let X = 1-alpha
            # (1-G^2)X^2 + 2kG^2 X - 2kG^2 = 0
            
            # If G=1, handled above.
            
            A = 1.0 - G*G
            B = 2.0 * k * G*G
            C = -2.0 * k * G*G
            
            # Quadratic formula for X: (-B + sqrt(B^2 - 4AC)) / 2A
            # X must be positive and usually near 1 for small alpha
        ...
        if np.isscalar(slope):
            ...
            alpha = np.zeros_like(slope)
            # Optimize: only compute for non-zero
            mask = slope > 1e-6
            if np.any(mask):
                s_masked = slope[mask]
                G = 10.0 ** (-s_masked / 20.0)
                A = 1.0 - G*G
                B = 2.0 * k * G*G
                C = -2.0 * k * G*G
                delta = B*B - 4*A*C
        ...
        self.output[0] = (1-alpha[0])*self.input[0] if not np.isscalar(alpha) else (1-alpha)*self.input[0]
        
        N = self.mast.params["N_SAMP"]
        out = self.output
        inp = self.input
