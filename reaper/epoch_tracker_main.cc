...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-i" && (i + 1) < argc) { filename = argv[++i]; }
    else if (arg == "-f" && (i + 1) < argc) { f0_output = argv[++i]; }
    else if (arg == "-p" && (i + 1) < argc) { pm_output = argv[++i]; }
    else if (arg == "-c" && (i + 1) < argc) { corr_output = argv[++i]; }
    else if (arg == "-t") { do_hilbert_transform = true; }
    else if (arg == "-s") { do_high_pass = false; }
    else if (arg == "-e" && (i + 1) < argc) { external_frame_interval = static_cast<float>(atof(argv[++i])); }
    else if (arg == "-x" && (i + 1) < argc) { max_f0 = static_cast<float>(atof(argv[++i])); }
    else if (arg == "-m" && (i + 1) < argc) { min_f0 = static_cast<float>(atof(argv[++i])); }
    else if (arg == "-u" && (i + 1) < argc) { inter_pulse = static_cast<float>(atof(argv[++i])); }
    else if (arg == "-w" && (i + 1) < argc) { unvoiced_cost = static_cast<float>(atof(argv[++i])); }
    else if (arg == "-a") { ascii = true; }
    else if (arg == "-d" && (i + 1) < argc) { debug_output = argv[++i]; }
    else if (arg == "-h") { fprintf(stdout, "\n%s\n", kHelp); return 0; }
  }
